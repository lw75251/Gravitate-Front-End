...
//        JSONObject deleteJSON = JSONUtils.deleteRideRequestJSON(user_id, event_id, ride_request_id);
    ...
    */
    ...
    /* Sends a GET Request to server to retrieve Event Details
     *   RETURNS: JSONObject that contains details
     */
    public static void getEvent(final Context ctx, final String eventId, final String token, final VolleyCallback callback) {
        Uri.Builder builder = new Uri.Builder();
        builder.scheme("https")
                .path(backendUrl + "/events")
                .appendPath(eventId);
        final String request_url = builder.toString();
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest
                (Request.Method.GET, request_url, null, new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        callback.onSuccessResponse(response);
                        Log.w("GETEvent", response.toString());
                    }
                }, new Response.ErrorListener() {
        ...
        APIRequestSingleton.getInstance(ctx).addToRequestQueue(jsonObjectRequest, "getEvent");
    }
